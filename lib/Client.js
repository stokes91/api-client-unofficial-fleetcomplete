...
class Client {
  ...
  getAuthenticationCode(callback) {
    const serializedRequest = Buffer.from(
      JSON.stringify({
        email: this.Email,
    ...
    const req = https.request(
      {
        ...
        method: "POST",
        path:
          "/Authentication/v10/ApplicationToken.svc/refreshApplicationToken/",
        headers: Object.assign(
          {
  ...
  getPOIGeofences(callback) {
    const req = https.request(
      {
        host: "tlshosted.fleetcomplete.com",
        method: "GET",
        path: "/MicroService/POI/v1/api/geofences",
        headers: Object.assign(
          {
            AppToken: this.ApplicationToken,
            "Content-Type": "application/json;v=1.0",
      ...
      (res) => {
        ...
        res.on("end", () => {
          try {
            this.poiGeofences = JSON.parse(Buffer.concat(b).toString());
          } catch (e) {
            callback(true);
            return;
          }
          if (!this.poiGeofences || !this.poiGeofences.length) {
            callback(true);
          }
